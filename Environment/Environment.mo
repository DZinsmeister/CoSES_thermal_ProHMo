...
within CoSES_Models.Environment;
model Environment "Environment with user defined weather profiles"
	...
	public
		Modelica.Blocks.Sources.RealExpression TimeBase(y=if useDayTime then if useHours then HourOfDay else HourOfDay*3600 else if useHours then HourOfYear else HourOfYear*3600) "Set output signal to a time varying Real expression" annotation(Placement(transformation(extent={{-60,65},{-40,85}})));
...
; 7 - relative Humidity [%]" annotation(Placement(transformation(extent={{0,65},{20,85}})));
	...
	public
		Real relHumidityAir(
			...
			tab="Results 2",
			visible=false));
		parameter Boolean ReferenceDay=false "If true: Data from reference day/week" annotation(Dialog(
			group="Weather Input Data",
			tab="Input Data"));
		parameter String InputFile=classDirectory()+"Data\\Weather_2021_Munich_15min.txt" "File name for weather data" annotation(Dialog(
			group="Weather Input Data",
			tab="Input Data"));
		parameter String TableName="Weather" "Structure: time [s], Ambient temperature [°C], Solar direct radiation in radiation direction [W/m²], Solar diffuse radiation [W/m²], Wind speed [m/s], Wind direction [°]" annotation(Dialog(
			group="Weather Input Data",
			tab="Input Data"));
	...
	protected
		parameter Real StandardAlpha(
			...
			tab="Time and Location"));
	public
		parameter Boolean DefaultInit=true if not ReferenceDay "Use default data for time initialization" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
	protected
		parameter Boolean useDayTime=false if not (DefaultInit or ReferenceDay) "If enabled, database = HourOfDay, else database = HourOfYear" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
		parameter Boolean useHours=false if not (DefaultInit or ReferenceDay) "If enabled, database = HourOfDay(max. 24h) or HourOfYear(max. 8760h), else database = HourOfDay(max. 86400s) or HourOfYear(max. 31536000s)" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
	public
		parameter Boolean useUnixTimeStamp=false if not (DefaultInit or ReferenceDay) "Time base is calculated via unix time stamp input in SimulationX's Property Dialog" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
		parameter Integer DayTimeInit=0 if not useUnixTimeStamp and not (DefaultInit or ReferenceDay) "Initial time of day" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
		parameter Integer WeekDayInit=6 if not useUnixTimeStamp and not (DefaultInit or ReferenceDay) "Initial day of week" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
		parameter Integer MonthDayInit=1 if not useUnixTimeStamp and not (DefaultInit or ReferenceDay) "Initial day of Month" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
		parameter Integer MonthInit=1 if not useUnixTimeStamp and not (DefaultInit or ReferenceDay) "Initial Month of year" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
		parameter Integer YearInit=2020 if not useUnixTimeStamp and not (DefaultInit or ReferenceDay) "Initial year" annotation(Dialog(
			group="Initialization",
			tab="Time and Location"));
	protected
		parameter Boolean DST=if useUnixTimeStamp then true else useSummerTime "Daylight Saving Time enabled" annotation(Dialog(
			group="Daylight Saving Time",
			tab="Time and Location"));
	public
		parameter Boolean useSummerTime=false if not useUnixTimeStamp "If enabled, Summer time is used for calculation of actual time" annotation(Dialog(
			group="Daylight Saving Time",
			tab="Time and Location"));
		parameter Integer SummerTimeBegin=85 if not useUnixTimeStamp and DST "Day in year when Summer time begins" annotation(Dialog(
			group="Daylight Saving Time",
			tab="Time and Location"));
		parameter Integer SummerTimeEnd=302 if not useUnixTimeStamp and DST "Day in year when Summer time ends" annotation(Dialog(
			group="Daylight Saving Time",
			tab="Time and Location"));
	...
	public
		Modelica.Blocks.Tables.CombiTable1D Init(
			...
			tableName="Init",
			fileName=InputFile,
			columns={2,3,4,5,6}) "1 - Day Time Init
2 - Week Day Init
3 - Month Day Init
4 - Month Init
5 - Year Init" annotation(Placement(transformation(extent={{0,0},{20,20}})));
	...
	initial equation
		...
		if useUnixTimeStamp then
			(HourOfDay, DayOfWeek, DayOfMonth, MonthOfYear, DayOfYear, Year, DSTstartDay, DSTendDay) = initTimeBaseUnix(time);
		
		elseif (DefaultInit or ReferenceDay) then
			assert((Init.y[1]>=0) and (Init.y[1]<24), "Time of day out of plausible limits");
			assert((Init.y[3]>=1) and (Init.y[3]<=Days[int(Init.y[4])]), "Day of Month out of plausible limits");
			assert((Init.y[4]>=1) and (Init.y[4]<=12), "Month out of plausible limits");
			assert(Init.y[5]>=1, "Year out of plausible limits");
			assert(Init.y[2]>=1 and Init.y[2]<=7, "Day of week out of plausible limits");
			
			HourOfDay = Init.y[1];
			DayOfWeek=Init.y[2]+HourOfDay/24;
			DayOfMonth=Init.y[3]+HourOfDay/24;
			MonthOfYear=Init.y[4]+((Init.y[3]+HourOfDay/24)-1)/Days[int(Init.y[4])];
			if DST then	
				DSTstartDay=SummerTimeBegin;
				DSTendDay=SummerTimeEnd;
			else
				DSTstartDay=-1;
				DSTendDay=-1;
			end if;
			
			if (not SummerTime) then	
				if (Init.y[4]>1) then
					DayOfYear=Init.y[3]+HourOfDay/24+sumOfDays(Days,1,int(Init.y[4])-1)-1;
					Year=Init.y[5]+(Init.y[3]+HourOfDay/24-1)/(sumOfDays(Days,1,12))+(sumOfDays(Days,1,int(Init.y[4])-1))/(sumOfDays(Days,1,12));
				else
					DayOfYear=Init.y[3]+HourOfDay/24-1;
					Year=Init.y[5]+(Init.y[3]+HourOfDay/24-1)/(sumOfDays(Days,1,12));
				end if;
			else
				if (Init.y[3]>1) then
					if (Init.y[4]>1) then
						DayOfYear=Init.y[3]+(HourOfDay-1)/24+sumOfDays(Days,1,int(Init.y[4])-1)-1;
						Year=Init.y[5]+(Init.y[3]+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12))+(sumOfDays(Days,1,int(Init.y[4])-1))/(sumOfDays(Days,1,12));
					else
						DayOfYear=Init.y[3]+(HourOfDay-1)/24-1;
						Year=Init.y[5]+(Init.y[3]+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					end if;
				else
					if (Init.y[4]>2) then
						DayOfYear=Days[int(Init.y[4]-1)]+(HourOfDay-1)/24+sumOfDays(Days,1,int(Init.y[4])-2);
						Year=Init.y[5]+(Days[int(Init.y[4]-1)]+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					elseif (int(Init.y[4])==2) then
						DayOfYear=Days[1]+(HourOfDay-1)/24;
						Year=Init.y[5]+(Days[1]+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					else
						DayOfYear=sumOfDays(Days,1,12)+(HourOfDay-1)/24;
						Year=Init.y[5]+((HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					end if;
				end if;		
			end if;
		else		
			...
			if DST then
				assert(DSTstartDay>=0 and DSTstartDay<=sumOfDays(Days,1,12) and DSTendDay>=0 and DSTendDay<=sumOfDays(Days,1,12),"Summer time days out of boundaries");
				if beta>0 then
					assert(DSTstartDay<DSTendDay,"Begin of summer time must be earlier than end");
				else
					assert(DSTstartDay>DSTendDay,"Begin of summer time must be later than end");
				end if;
			end if;			
			
			assert((DayTimeInit>=0) and (DayTimeInit<24), "Time of day out of plausible limits");
			assert((MonthDayInit>=1) and (MonthDayInit<=Days[MonthInit]), "Day of Month out of plausible limits");
			assert((MonthInit>=1) and (MonthInit<=12), "Month out of plausible limits");
			assert(YearInit>=1, "Year out of plausible limits");
			assert(WeekDayInit>=1 and WeekDayInit<=7, "Day of week out of plausible limits");		
		
			if (DST and DayTimeInit==2) then
				if MonthInit>1 then
					if (integer(MonthDayInit+DayTimeInit/24+sumOfDays(Days,1,MonthInit-1)-1)==DSTstartDay) then	
						HourOfDay=3;
					else
						HourOfDay=DayTimeInit;
					end if;
				else
					if (integer(MonthDayInit+DayTimeInit/24-1)==DSTstartDay) then
						HourOfDay=3;
					else
						HourOfDay=DayTimeInit;
					end if;
			...
			DayOfWeek=WeekDayInit+HourOfDay/24;
			DayOfMonth=MonthDayInit+HourOfDay/24;
			MonthOfYear=MonthInit+((MonthDayInit+HourOfDay/24)-1)/Days[MonthInit];
				
			if (not SummerTime) then	
				if (MonthInit>1) then
					DayOfYear=MonthDayInit+HourOfDay/24+sumOfDays(Days,1,MonthInit-1)-1;
					Year=YearInit+(MonthDayInit+HourOfDay/24-1)/(sumOfDays(Days,1,12))+(sumOfDays(Days,1,MonthInit-1))/(sumOfDays(Days,1,12));
				else
					DayOfYear=MonthDayInit+HourOfDay/24-1;
					Year=YearInit+(MonthDayInit+HourOfDay/24-1)/(sumOfDays(Days,1,12));
				end if;
			else
				if (MonthDayInit>1) then
					if (MonthInit>1) then
						DayOfYear=MonthDayInit+(HourOfDay-1)/24+sumOfDays(Days,1,MonthInit-1)-1;
						Year=YearInit+(MonthDayInit+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12))+(sumOfDays(Days,1,MonthInit-1))/(sumOfDays(Days,1,12));
					else
						DayOfYear=MonthDayInit+(HourOfDay-1)/24-1;
						Year=YearInit+(MonthDayInit+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					end if;
				else
					if (MonthInit>2) then
						DayOfYear=Days[MonthInit-1]+(HourOfDay-1)/24+sumOfDays(Days,1,MonthInit-2);
						Year=YearInit+(Days[MonthInit-1]+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					elseif (MonthInit==2) then
						DayOfYear=Days[1]+(HourOfDay-1)/24;
						Year=YearInit+(Days[1]+(HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					else
						DayOfYear=sumOfDays(Days,1,12)+(HourOfDay-1)/24;
						Year=YearInit+((HourOfDay-1)/24-1)/(sumOfDays(Days,1,12));
					end if;
				end if;		
			end if;
		end if;
	...
	equation
		...
		if useDayTime then
			SecondOfYear = HourOfYear * 3600;
		else
			SecondOfYear = HourOfDay * 3600;
		end if;
		...
		if noEvent(SmoothData) then
			...
			if (not(useDayTime) or (HourOfDay<23.5 and HourOfDay>0.5)) then
	...
	equation
		...
		connect(TimeBase.y,WeatherData.u[5]) annotation(Line(
			...
			color={0,0,127},
			thickness=0.0625));
		connect(realExpression1.y,Init.u[1]) annotation(Line(
			points={{-19,10},{-14,10},{-7,10},{-2,10}},
			color={0,0,127},
			thickness=0.0625));
		connect(realExpression1.y,Init.u[2]) annotation(Line(
			points={{-19,10},{-14,10},{-7,10},{-2,10}},
			color={0,0,127},
			thickness=0.0625));
		connect(realExpression1.y,Init.u[3]) annotation(Line(
			points={{-19,10},{-14,10},{-7,10},{-2,10}},
			color={0,0,127},
			thickness=0.0625));
		connect(realExpression1.y,Init.u[4]) annotation(Line(
			points={{-19,10},{-14,10},{-7,10},{-2,10}},
			color={0,0,127},
			thickness=0.0625));
		connect(realExpression1.y,Init.u[5]) annotation(Line(
			points={{-19,10},{-14,10},{-7,10},{-2,10}},
			color={0,0,127},
			thickness=0.0625));
